    ...
    internal class SiloStatisticsManager
    {
        ...
        internal async Task SetSiloMetricsTableDataManager(Silo silo, StatisticsOptions options)
        {
            ...
            else if (CanUseAzureTable(silo, options))
            {
                ...
                await metricsDataPublisher.Init(this.siloDetails.ClusterId, this.storageOptions.DataConnectionString, this.siloDetails.HostSiloAddress ?? this.siloDetails.SiloAddress, this.siloDetails.Name, gateway, this.siloDetails.DnsHostName);
                MetricsTable.MetricsDataPublisher = metricsDataPublisher;
            }
        }
        ...
        internal async Task SetSiloStatsTableDataManager(Silo silo, StatisticsOptions options)
        {
            ...
            else if (CanUseAzureTable(silo, options))
            {
                ...
                await statsDataPublisher.Init(true, this.storageOptions.DataConnectionString, this.siloDetails.ClusterId, (this.siloDetails.HostSiloAddress ?? this.siloDetails.SiloAddress).ToLongString(), this.siloDetails.Name, this.siloDetails.DnsHostName);
                logStatistics.StatsTablePublisher = statsDataPublisher;
            }
            // else no stats
        }
