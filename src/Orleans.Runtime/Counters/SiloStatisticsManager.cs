    ...
    internal class SiloStatisticsManager
    {
        ...
        internal async Task SetSiloStatsTableDataManager(Silo silo, NodeConfiguration nodeConfig)
        {
            ...
            else if (useAzureTable)
            {
                ...
                await statsDataPublisher.Init(true, silo.GlobalConfig.DataConnectionString, this.siloDetails.ClusterId, (this.siloDetails.HostSiloAddress ?? this.siloDetails.SiloAddress).ToLongString(), this.siloDetails.Name, this.siloDetails.DnsHostName);
                logStatistics.StatsTablePublisher = statsDataPublisher;
            }
            // else no stats
        }
