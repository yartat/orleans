...
namespace Orleans.Runtime.Placement
{
    internal class StatelessWorkerDirector : IPlacementDirector, IActivationSelector
    {
        ...
        {
            return Task.FromResult(context.LocalHostSilo ?? context.LocalSilo);
        }
        ...
        private PlacementResult SelectActivationCore(PlacementStrategy strategy, GrainId target, IPlacementRuntime context)
        {
            ...
            foreach (var activation in local)
            {
                ...
                return PlacementResult.IdentifySelection(ActivationAddress.GetAddress(context.LocalHostSilo ?? context.LocalSilo, target, activation.ActivationId));
            }

            if (local.Count >= placement.MaxLocal)
            {
                var id = local[local.Count == 1 ? 0 : random.Next(local.Count)].ActivationId;
                return PlacementResult.IdentifySelection(ActivationAddress.GetAddress(context.LocalHostSilo ?? context.LocalSilo, target, id));
            }
