        ...
        ILocalGrainDirectory, ISiloStatusListener
    {
            ...
            ILoggerFactory loggerFactory)
        {
            ...
            */
            ...
            {
                this.seed = this.MyHostAddress != null ?
                    this.MyHostAddress.Endpoint.Equals(primarySiloEndPoint) ? this.MyHostAddress : SiloAddress.New(primarySiloEndPoint, 0) :
                    this.MyAddress.Endpoint.Equals(primarySiloEndPoint) ? this.MyAddress : SiloAddress.New(primarySiloEndPoint, 0);
            }
        ...
        protected void AddServer(SiloAddress silo)
        {
            lock (membershipCache)
            {
                ...
                if (log.IsEnabled(LogLevel.Debug)) log.Debug("Silo {0}{1} added silo {2}", 
        ...
        protected void RemoveServer(SiloAddress silo, SiloStatus status)
        {
            lock (membershipCache)
            {
                ...
                if (log.IsEnabled(LogLevel.Debug)) log.Debug("Silo {0}{1} removed silo {2}", 
                    MyAddress, MyHostAddress != null ? $"(hosted in {MyHostAddress})" : string.Empty, silo);
            }
        }
        ...
        public SiloAddress CalculateTargetSilo(GrainId grainId, bool excludeThisSiloIfStopping = true)
        {
            ...
            {
                if (log.IsEnabled(LogLevel.Trace)) log.Trace("Silo {0}{1} looked for a system target {2}, returned {3}", 
            ...
            if (Constants.SystemMembershipTableId.Equals(grainId))
            {
                ...
                if (log.IsEnabled(LogLevel.Trace)) log.Trace("Silo {0}{1} looked for a special grain {2}, returned {3}",
            ...
            if (log.IsEnabled(LogLevel.Trace)) log.Trace("Silo {0}{1} calculated directory partition owner silo {2} for grain {3}: {4} --> {5}", 
        ...
        public bool LocalLookup(GrainId grain, out AddressesAndTag result)
        {
            ...
            if (log.IsEnabled(LogLevel.Debug)) log.Debug("Silo {0}{1} tries to lookup for {2}-->{3} ({4}-->{5})", 
