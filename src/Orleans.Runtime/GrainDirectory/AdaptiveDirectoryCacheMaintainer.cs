    ...
    internal class AdaptiveDirectoryCacheMaintainer<TValue> : AsynchAgent
    {
        ...
        protected override void Run()
        {
            while (router.Running)
            {
                ...
                if (Log.IsEnabled(LogLevel.Trace)) Log.Trace("Silo {0}{1} self-owned (and removed) {2}, kept {3}, removed {4} and tries to refresh {5} grains", 
        ...
        private void SendBatchCacheRefreshRequests(Dictionary<SiloAddress, List<GrainId>> refreshRequests)
        {
            foreach (SiloAddress silo in refreshRequests.Keys)
            {
                ...
                if (Log.IsEnabled(LogLevel.Trace)) Log.Trace("Silo {0}{1} is sending request to silo {2} with {3} entries", 
        ...
        {
            if (Log.IsEnabled(LogLevel.Trace)) Log.Trace("Silo {0}{1} received ProcessCacheRefreshResponse. #Response entries {2}.", 
            ...
            if (Log.IsEnabled(LogLevel.Trace)) Log.Trace("Silo {0}{1} processed refresh response from {2} with {3} updated, {4} removed, {5} unchanged grains", 
