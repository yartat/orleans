...
namespace Orleans.Clustering.ServiceFabric
{
    ...
    internal class FabricMembershipOracle : IMembershipOracle, IFabricServiceStatusListener, IDisposable
    {
            ...
            UnknownSiloMonitor unknownSiloMonitor)
        {
            ...
            if (this.HostSiloAddress != null)
                this.silos[this.HostSiloAddress] = new SiloEntry(SiloStatus.Created, this.SiloName);
        }

        /// <inheritdoc />
        public SiloStatus CurrentStatus => this.GetApproximateSiloStatus(this.HostSiloAddress ?? this.SiloAddress);

        /// <inheritdoc />
        public SiloAddress SiloAddress => this.localSiloDetails.SiloAddress;

        /// <inheritdoc />
        public SiloAddress HostSiloAddress => this.localSiloDetails.HostSiloAddress;
        ...
        {
            if (address.Equals(this.HostSiloAddress) || address.Equals(this.SiloAddress)) return false;
        ...
        {
            if (siloAddress.Equals(this.HostSiloAddress) || siloAddress.Equals(this.SiloAddress)) return true;
        ...
        public void OnUpdate(FabricSiloInfo[] partitions)
        {
            ...
            lock (this.updateLock)
            {
                foreach (var updatedSilo in partitions)
                {
                    ...
                    SiloEntry existing;
                    if (this.silos.TryGetValue(updatedSilo.HostSiloAddress, out existing))
                    {
                        ...
                        if (existing.Status != SiloStatus.Active)
                        {
                            this.log.Error(
                                ...
                                $"Encountered status update indicating a silo which was previously declared dead is now active. Name: {existing.Name}, Address: {updatedSilo.HostSiloAddress}");
                        }
                    }
                    else if (this.silos.TryGetValue(updatedSilo.SiloAddress, out existing))
                    {
                        // Mark the existing silo as being refreshed.
                        existing.Refreshed = true;
                    ...
                    {
                        // Add the new silo.
                        this.notifications.Add(new StatusChangeNotification(updatedSilo.HostSiloAddress ?? updatedSilo.SiloAddress, SiloStatus.Active));
                        this.silos[updatedSilo.SiloAddress] = new SiloEntry(SiloStatus.Active, updatedSilo.Name)
                        {
                            Refreshed = true
                        };

                        if (updatedSilo.HostSiloAddress != null)
                        {
                            this.silos[updatedSilo.HostSiloAddress] = new SiloEntry(SiloStatus.Active, updatedSilo.Name)
                            {
                                Refreshed = true
                            };
                        }
                ...
                foreach (var silo in this.silos)
                {
                    ...
                    if (silo.Key.Equals(this.HostSiloAddress) || silo.Key.Equals(this.SiloAddress)) continue;
        ...
        private void UpdateStatus(SiloStatus status)
        {
            ...
            lock (this.updateLock)
            {
                if (this.HostSiloAddress == null || !this.silos.TryGetValue(this.HostSiloAddress, out var existing))
                    existing = this.silos[this.SiloAddress];
                ...
                if (existing.Status != status)
                {
                    ...
                    if (this.HostSiloAddress != null)
                        this.silos[this.HostSiloAddress] = new SiloEntry(status, this.SiloName);
                    this.ClearCaches();
                }
            }
            ...
            if (updated)
            {
                foreach (var subscriber in this.subscribers.Values)
                {
                    subscriber.SiloStatusChangeNotification(this.HostSiloAddress ?? this.SiloAddress, status);
                }
            }
        }
