...
namespace Orleans.Runtime
{
    ...
    public abstract class GrainService : SystemTarget, IRingRangeListener, IGrainService
    {
        ...
        protected GrainService() : base(null, null, null, null)
        {
            throw new Exception("This should not be constructed by client code.");
        }

        /// <summary>Constructor to use for grain services</summary>
        protected GrainService(IGrainIdentity grainId, Silo silo, IGrainServiceConfiguration config, ILoggerFactory loggerFactory) : base((GrainId)grainId, silo.SiloAddress, lowPriority: true, hostSilo: silo.SiloHostAddress, loggerFactory:loggerFactory)
        {
